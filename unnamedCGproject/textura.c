...
GLuint loadTexture(const char *filename){
    ...
    // largura, altura -> dimens�es da imagem
    ...
    if(!dados){
        ...
        return 0;
    }//facilita pra encontrar o problema caso aconte�a
    ...
    //Filtro de minifica��o e magnifica��o, serve para ajustar quando a imagem/textura for maior/menor que a textura original
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR); //usa interpola��o bilineal para reduzir a imagem
...
/*
    GLenum formato = (canais == 4) ? GL_RGBA : GL_RGB;//Serve para identificar se � uma png ou jpg

    glTexImage2D(GL_TEXTURE_2D, // Tipo de textura
             0,                // N�vel mipmap, ai colocamos a textura original
...
*/
    ...
    //gera vers�es menores das texturas para os objetos longe da c�mera, e deixa a imagem melhor
 //   gluBuild2DMipmaps(GL_TEXTURE_2D, formato, largura, altura, formato, GL_UNSIGNED_BYTE, dados);

    stbi_image_free(dados); //libera a informa��es dos dados da imagem para o opengl usar
